...
Train_Data <- read.csv("train_v2.csv", stringsAsFactors=FALSE)
...
#################################################################
#Mehta - picks up from line 318
###############################################################

#Remove highly correlated variables 
hc <- cor(imputed_data)
hc1 <- findCorrelation(df3,cutoff=0.99)
hc1 <- sort(hc1)
df3 <- imputed_data[,-c(hc1)]
#473 variables remaining

df3$default <- ifelse(df3$loss>0,1,0)

table(df3$default)

set.seed(998)
inTraining = createDataPartition(df3$default, p = 0.70, list = F)
...
training <- training[,-1]
testing <- testing[,-1]
####################### Model Testing for Default Classification #############

#PCA: Not currently using..

#Run PCA 
pr.out <- prcomp(training, scale=TRUE)
...
#Looks like we may be able to reduce the variable count down to ~200 variables using PCA


############### Random Forest ###########
set.seed(998)
rf <- randomForest(as.factor(default) ~ ., data=training[,-c(1,472)], importance=TRUE, ntree=50) 
plot(rf)
varImpPlot(rf)

randomforest.pred <- predict(rf, testing)
table(randomforest.pred,testing$default)

#strange results: lots of 0's predicted.

############ Logistic Regression ###########
training1 <- training[,-c(1,472,473)]
log.reg <- glm(training$default ~ ., data=training1, family = binomial)
#Warning message:
 # glm.fit: fitted probabilities numerically 0 or 1 occurred 

log.reg.pred <- predict(log.reg,testing, type="response")
default.pred <- ifelse(log.reg.pred >= 0.5,1,0)
table(default.pred,testing$default)
###plot ROC curve and calculate Area under curve
library(AUC)
roc_data <- cbind(log.reg.pred,testing$default)
roc = roc(predictions = roc_data[,1] , labels = factor(testing$default))
auc= auc(roc)
#auc = 0.71
# plot curves
plot(roc, col = "red", lty = 1, lwd = 2, main = "ROC Curve")


############## Lasso Regression for Loss ###############

library (glmnet)
x <- model.matrix(loss~.,data=training)
x.test <- model.matrix(loss ~.,data=testing)
x.test <- x.test[x.test[,472] == 1,-c(1,472)]
x <- x[x[,472] == 1,-c(1,472)]
y <- training$loss[training$loss > 0]
y.test <- testing$loss[testing$loss > 0]
...
#################################################################
#Justin - picks up from line 318
###############################################################
library(randomForest)
library(readr)
train_v2 <- read_csv("~/train_v2.csv")
#Sampling
smpl2 <- train_v2[sample(nrow(train_v2), 10000),]
#check sampling by performing t test
t.test(train_v2$loss,smpl2$loss)


#impute missing values
imputed2 <- rfImpute(loss ~ ., smpl2,iter=5, ntree=300)

library(caret)


write.csv(smpl2, "sample.csv")
summary <- describe(smpl2, IQR = TRUE, quant=TRUE)

#Calculate number of missing records for each variable
missing_var <- nrow(smpl2) - summary$n
plot(missing_var)
#variable names missing more than 5000 records (may want to remove these. >5000 records is a lot of imputation)
high_missing <- colnames(smpl2)[missing_var >= 2000]

#[1] "f72"  "f159" "f160" "f169" "f170" "f179" "f180" "f189" "f190" "f199" "f200" "f209" "f210" "f330" "f331" "f340" "f341" "f422" "f586" "f587"
#[21] "f588" "f618" "f619" "f620" "f621" "f640" "f648" "f649" "f650" "f651" "f653" "f662" "f663" "f664" "f665" "f666" "f667" "f668" "f669" "f672"
#[41] "f673" "f679" "f726"

#Find columns where all values are the same (can remove these)
zero_cols <- rownames(summary)[summary$range==0 & summary$sd == 0]
# [1] "f33"  "f34"  "f35"  "f37"  "f38"  "f678" "f700" "f701" "f702" "f736" "f764"

#Get number of records that have no missing predictor variables
complete_records <- smpl2[complete.cases(smpl2),]
#51940 rows   49% of total
dropped<-smpl2[ , !(names(smpl2) %in% zero_cols)]
dropped2<-dropped[ , !(names(dropped) %in% high_missing)]
dropped2$default <- ifelse(dropped2$loss>0,1,0)
d<-c("loss")
dropped3<-dropped2[ ,!(names(dropped2) %in% d) ]
table(dropped2$default)

smpl2$default <- ifelse(smpl2$loss>0,1,0)
smpl3<-smpl2[ ,!(names(smpl2) %in% d) ]

rf.imputed <- randomForest(as.factor(default) ~ ., data=smpl3, importance=TRUE)
zero_cols <- rownames(summary)[summary$range==0 & summary$sd == 0]
# [1] "f33"  "f34"  "f35"  "f37"  "f38"  "f678" "f700" "f701" "f702" "f736" "f764"
...
imputed3<-imputed2[ , !(names(imputed2) %in% zero_cols)]
imputed3$default <- ifelse(imputed3$loss>0,1,0)
imputed3<-imputed3[ ,!(names(imputed3) %in% d) ]

#removed variables with any missing value or same range and sd.
rf.naive <- randomForest(loss~., data=imputed2, importance=TRUE) 
varImpPlot(rf.naive, n.var = 30)
varImp(rf.naive,scale=FALSE)
?varImpPlot

rf.imputed <- randomForest(as.factor(default) ~ ., data=imputed3, importance=TRUE)
varImp(rf.imputed)
varImpPlot(rf.imputed, n.var = 30)
write.csv(imputed3, "~/imputed.csv")


rf.imputed2 <- randomForest(as.factor(default) ~ ., data=imputed3, importance=TRUE)
varImp(rf.imputed2)
varImpPlot(rf.imputed2, n.var = 30)
